...
test("createRuntimeClient returns NestloneRuntimeClient instance", () => {
  const fetch = fakeFetch(() => jsonResponse({}));
  const client = createRuntimeClient({ fetch });

  assert.ok(client instanceof NestloneRuntimeClient);
...
test("worker and run actions use POST endpoints", async () => {
  ...
  const client = new NestloneRuntimeClient({ fetch });
