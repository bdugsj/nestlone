...
Nestlone is a Rust TUI and CLI for many model providers — DeepSeek,
...
Nestlone binaries for your platform, verifies them against the release
SHA-256 manifest, and installs the `nestlone`, `nest`, and `nestlone-tui`
commands. The application state and credentials still live in Nestlone's
normal config files, not inside `node_modules`.

> Previously published as `deepseek-tui`. See
> [docs/REBRAND.md](https://github.com/bdugsj/nestlone/blob/main/docs/REBRAND.md)
...
[docs/PROVIDERS.md](https://github.com/bdugsj/nestlone/blob/main/docs/PROVIDERS.md).

The `nestlone` facade and `nestlone-tui` binary share
`~/.nestlone/config.toml` for auth and default model settings. Legacy
...
`nestlone doctor`, `nestlone models`, `nestlone sessions`, and
`nestlone resume --last`.
...
Termux-native `nestlone`, `nest`, and `nestlone-tui` assets. That path works
...
table in [docs/INSTALL.md](https://github.com/bdugsj/nestlone/blob/main/docs/INSTALL.md).
...
[docs/INSTALL.md](https://github.com/bdugsj/nestlone/blob/main/docs/INSTALL.md)
guide.
...
| --- | --- |
| `nestloneBinaryVersion` in `package.json` | Default native binary version. `deepseekBinaryVersion` is still read as a backward-compat fallback. |
| `NESTLONE_RELEASE_BASE_URL` | Canonical override: use an internal or mirrored release-asset directory when GitHub Releases is unavailable. The directory must contain `nestlone-artifacts-sha256.txt` and the platform binaries. `CODEWHALE_RELEASE_BASE_URL`, `DEEPSEEK_TUI_RELEASE_BASE_URL`, and `DEEPSEEK_RELEASE_BASE_URL` are the implemented legacy fallbacks. |
| `NESTLONE_USE_CNB_MIRROR=1` | Download from the CNB (China-friendly) mirror on Linux x64 and OpenHarmony x64 (legacy `CODEWHALE_USE_CNB_MIRROR=1` also works). Other targets fail with a clear unsupported-mirror error; use GitHub or a complete `NESTLONE_RELEASE_BASE_URL` mirror there. |
| `DEEPSEEK_TUI_VERSION` or `DEEPSEEK_VERSION` | Override the GitHub release version to download. |
| `DEEPSEEK_TUI_GITHUB_REPO` or `DEEPSEEK_GITHUB_REPO` | Override the source repo. Defaults to `bdugsj/nestlone`. |
...
| `NESTLONE_SKIP_GLIBC_CHECK=1` | Bypass the Linux glibc preflight check at your own risk (`CODEWHALE_SKIP_GLIBC_CHECK=1` / `DEEPSEEK_SKIP_GLIBC_CHECK=1` also work). |
...
`NESTLONE_RELEASE_BASE_URL` at a local mirror.
...
- For the default GitHub Release source, `npm run release:check` also verifies
  ...
  the tag commit. When `NESTLONE_RELEASE_BASE_URL` or a legacy mirror override
...
- Website: <https://nestlone.net/>
- Provider registry: [docs/PROVIDERS.md](https://github.com/bdugsj/nestlone/blob/main/docs/PROVIDERS.md)
- Changelog: [CHANGELOG.md](https://github.com/bdugsj/nestlone/blob/main/CHANGELOG.md)
