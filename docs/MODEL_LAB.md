...
Model Lab is the planned open-model workbench for Nestlone. The north star is
simple: Nestlone should make open-source and open-weight models practical in
...
- OpenRouter, Novita, Fireworks, NVIDIA NIM, AtlasCloud, Wanjie Ark, Hugging
  ...
  `/provider`, `nestlone --provider`, or `nestlone models`.
...
describe a model as available before Nestlone can actually route to it.
