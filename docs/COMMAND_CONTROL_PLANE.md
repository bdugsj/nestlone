...
Nestlone exposes the same lifecycle operations on three surfaces: a slash
...
showed the current session's sub-agents while `nestlone fleet status` read the
...
`nestlone-lane` is the lowest crate the thin CLI facade and the TUI both
...
- `surface_not_supported` — the backend exists but not here. The hint names the
  surface that works (`nestlone fleet restart <worker-id>`).
...
```
nestlone lane interrupt lane-a1b2c3d4@3
