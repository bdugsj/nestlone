...
Hooks run a shell command when the Nestlone **TUI** reaches a lifecycle
...
| Surface | Fires hooks |
| --- | --- |
| `nestlone` / `nest` interactive TUI | yes |
| `nestlone exec` (headless one-shot) | no |
| the `nestlone` CLI dispatcher and its subcommands | no |
| app-server / ACP | no |
...
name = "announce"
event = "session_start"
command = "echo 'Nestlone session started'"
```
...
command = "~/.nestlone/hooks/gate.sh"  # required; `sh -c` on Unix, `cmd /C` on Windows
...
**Termination is best-effort, and the bound that is guaranteed is Nestlone's,
...
Nestlone does guarantee is that it stops waiting: the containment handle is
...
A hook command inherits the environment of the Nestlone process, plus the
`DEEPSEEK_*` variables for its event. Nestlone does not filter that
inheritance, so treat a hook exactly as you would treat any command you type
in the same shell that launched Nestlone: whatever is exported there is
visible to it.
...
- **Unsupported conditions are rejected at load.** A condition that references
  ...
  believes is armed. Nestlone drops those hooks at load, logs the reason
...
A repository may ship `<workspace>/.nestlone/hooks.toml` using the same shape,
...
trusted project file logs a warning and Nestlone falls back to global hooks
...
Observer means Nestlone ignores the hook's **result**: stdout is discarded, a
...
is change what Nestlone itself does next.
...
- `background = true` hooks are submitted and never awaited, so they have no
  verdict and cannot steer; Nestlone logs a warning when one is configured
  for this event
...
Nestlone's ambient environment. Its environment is built as:
...
Nestlone are **not** forwarded to a local `exec_shell` on their own.
...
backend, Nestlone does not construct the process environment at all: it hands
...
Nestlone's own audit log still records key names only, but that says nothing
...
Resolved **key names — never values** — are written to `~/.nestlone/audit.log`
...
- Hooks are arbitrary shell commands from your own config; treat
  `~/.nestlone/config.toml` as executable.
- Project-supplied hooks require an explicit workspace trust decision in
  user-owned config.
- Hook commands inherit Nestlone's own environment. A local `exec_shell` does
  not — see [`shell_env`](#shell_env).
- `shell_env` audit records contain key names only. That covers Nestlone's own
...
- Nothing Nestlone persists in a denial echoes the stdin payload, hook
