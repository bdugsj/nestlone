...
Nestlone codebase. It is intentionally conservative: shipped entries are
...
- `crates/agent/src/lib.rs` - static `ModelRegistry` used by
  `nestlone model list` and `nestlone model resolve`.
...
Fresh shared config writes to `~/.nestlone/config.toml`. Existing
...
`nestlone --provider <id>`. Set the request model with `CODEWHALE_MODEL`, a
...
with `nestlone auth set --provider <id>`, `[providers.<table>].api_key`, or
...
For hosted providers, `nestlone auth set --provider <id>` saves an API key for
...
ID; Nestlone does not infer a switch to OpenRouter, DeepSeek, or another
provider from the `qwen` prefix. Set `context_window` to the gateway/model's
real total context window when it differs from Nestlone's static model
metadata.

Private gateways with broken or intercepted certificates should use
`SSL_CERT_FILE` with a trusted CA bundle. The legacy
`insecure_skip_tls_verify = true` key is still parsed so `nestlone doctor` can
...
| `openai-codex` | Run `codex login`, then explicitly grant Nestlone read-only access to that exact credential file; no Nestlone API key is stored. |
...
| `xai` | [xAI Console](https://console.x.ai/) for an API key, Nestlone-owned device login, or explicitly consented read-only Grok CLI credentials. |
...
secret. Nestlone links straight to that console and accepts the copied key.
It never probes or impersonates `kimi_cli`/`kimi_code_cli`; first-class Kimi
OAuth remains blocked on a vendor-registered Nestlone identity.
...
Nestlone currently supports exact-path, provider-scoped **read-only** grants
for the Codex CLI and Grok CLI:

```bash
codex login
nestlone auth external-consent --provider openai-codex --mode read-only

grok login
nestlone auth external-consent --provider xai --mode read-only

nestlone auth status --provider openai-codex
...
yet preserve every unknown external schema field safely. Nestlone-started xAI
device login instead atomically activates a Nestlone-owned generation named
...
| `xiaomi-mimo` | `[providers.xiaomi_mimo]` | `XIAOMI_MIMO_TOKEN_PLAN_API_KEY`, `MIMO_TOKEN_PLAN_API_KEY`, `XIAOMI_MIMO_API_KEY`, `XIAOMI_API_KEY`, `MIMO_API_KEY` | `XIAOMI_MIMO_BASE_URL`, `MIMO_BASE_URL`, `XIAOMI_MIMO_MODE`, `MIMO_MODE`; default `https://token-plan-sgp.xiaomimimo.com/v1` | Chat: `mimo-v2.5-pro`, `mimo-v2.5-pro-ultraspeed`, `mimo-v2.5`; speech/TTS: `mimo-v2.5-tts`, `mimo-v2.5-tts-voicedesign`, `mimo-v2.5-tts-voiceclone`, `mimo-v2-tts` | Xiaomi MiMo OpenAI-compatible chat completions route. Token Plan keys (`tp-...`) use `api-key` auth and the token-plan endpoint by default; pay-as-you-go mode uses standard API keys (`sk-...`) and `https://api.xiaomimimo.com/v1`. It sends `max_completion_tokens` and uses MiMo's `thinking` field for reasoning control. Token Plan cost/usage is credit/quota based; Nestlone shows it as unknown until Xiaomi exposes a reliable balance API. `nestlone speech` / `tts` uses the TTS models. |
...
| `xai` | `[providers.xai]` | `XAI_API_KEY`, Nestlone-owned device OAuth, or explicit read-only Grok CLI consent | `XAI_BASE_URL`; default `https://api.x.ai/v1` | `grok-4.5` (default), `grok-4.3`, `grok-build`, `grok-composer-2.5-fast`, `grok-4.20-0309-reasoning`, `grok-4.20-0309-non-reasoning` | xAI/Grok OpenAI-compatible Chat Completions route. **API-key** (default): Bearer token from console.x.ai via `XAI_API_KEY` / keyring / `api_key`. **OAuth**: `nestlone auth xai-device` uses SSH-friendly device login and Nestlone-owned storage, which may refresh itself. Existing Grok CLI credentials require `nestlone auth external-consent --provider xai --mode read-only`; the granted external file is never refreshed or rewritten. OAuth may return HTTP 403 on some SuperGrok tiers — keep API-key as the reliable fallback. `XAI_MODEL` is accepted. Provider aliases: `x-ai`, `x_ai`, `grok`. |
...
Nestlone's `huggingface` provider ID is only the OpenAI-compatible chat
...
`nestlone --provider xiaomi-mimo speech "text" --model tts` (or the `tts`
...
Plan docs and console expose credit/quota semantics, but Nestlone does not
...
includes `mimo-v2.5` for image input. Nestlone exposes image analysis through the
...
Nestlone preserves the `deepseek/` wire-model prefix under the OpenRouter
...
fields, Nestlone treats them as absent for that response rather than as a
...
`nestlone model list` and `nestlone model resolve` use the static registry in
`crates/agent/src/lib.rs`. This is not the same as live `/models` discovery.
Use `/models` or `nestlone models` to fetch model IDs from the active API
...
| `xiaomi-mimo` | `mimo-v2.5-pro`, `mimo-v2.5-pro-ultraspeed`, `mimo-v2.5`; speech/TTS IDs are selected through `nestlone speech` / `tts` | yes | yes for chat models; no for speech/TTS models |
...
`nestlone-tui doctor --json` exposes the `capability` object. It is static
...
MiniMax M3 uses input-length and service tiers. Nestlone omits
...
disabled. Nestlone sends explicit controls when the user selects a reasoning
mode.
...
Nestlone only executes tools when the provider returns Chat Completions
...
- If in doubt, test a small `File` `read` or `search_content` action against a
  known tool-calling model before debugging Nestlone's tool registry.

Changing `provider`, `base_url`, or `model` can select a route that supports the
OpenAI-compatible payload shape, but Nestlone cannot convert arbitrary JSON
...
15:59 UTC. Nestlone migrates either name to `deepseek-v4-flash` before a
...
- **Direct Moonshot K3** (exact `https://api.moonshot.ai/v1` with
  ...
  The direct platform publishes no `off` state for K3, and Nestlone will not
...
Normal dispatched
`auto` uses Nestlone's auto-reasoning selector and sends a concrete tier;
...
- A unified `Provider` trait in `nestlone-agent` that owns env precedence,
