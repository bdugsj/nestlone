...
…or add to `~/.nestlone/config.toml`:
...
The memory file lives at `~/.nestlone/memory.md` by default; override
...
legacy fallback when no `.nestlone` memory file exists.
...
```xml
<user_memory source="/Users/you/.nestlone/memory.md">
...
`AGENTS.md`, `.nestlone/instructions.md`, legacy `.deepseek/instructions.md`,
and `instructions = [...]`.
...
The memory file lives entirely on your machine in `~/.nestlone/`.
...
`.nestlone/instructions.md` files instead. Legacy
...
| Memory file path      | `~/.nestlone/memory.md`       | `memory_path = "..."` or `DEEPSEEK_MEMORY_PATH=`  |
