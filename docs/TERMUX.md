...
Nestlone provides an Android arm64 build and archive path for
...
Or, when a release includes `nestlone-android-arm64.tar.gz`, extract it
into `$PREFIX/bin`.

> **Do not** install the GNU libc `nestlone-linux-arm64` archive in Termux.
> Android uses Bionic libc, not glibc — the Linux binary will not run.

## Platform behavior on Android

Nestlone's security model has three distinct layers on Android:
...
1. **Android's app sandbox** — Android assigns Termux its own app UID and
   ...
   Nestlone inherit that app boundary and any storage or other permissions the
...
2. **Nestlone's per-command sandbox backend** — Seatbelt (macOS) or the
   opt-in bubblewrap wrapper (Linux) can further narrow what a child command
   may access. Nestlone does not currently provide that additional layer on
   Android.
3. **Nestlone's own gates** — workspace trust, approval prompts,
...
### Nestlone sandbox backend: none

Nestlone's existing Seatbelt and Linux bubblewrap integrations do not target
Android. Consequently, `nestlone doctor --json` reports the sandbox as
`{"available": false, "kind": null}` on Android. That status describes the
absence of an additional Nestlone child-process sandbox; it does not mean
...
- Shell commands retain Termux's Android app boundary but receive no
  Nestlone-specific filesystem narrowing. Treat every location available to
  Termux, including user-granted shared storage, as potentially available to a
  command that you approve.

### Approvals: still apply

Nestlone's approval system (interactive prompts for risky actions,
...
Nestlone's Termux/native build has no supported OS keyring backend (the
desktop Secret Service/dbus integration is unavailable, and Nestlone does not
...
`~/.nestlone/secrets/` (Termux home directory), protected only by `0600`
...
- Keys saved through setup, `/provider`, or `nestlone auth set` are written to
  `~/.nestlone/config.toml` and mirrored to
  `~/.nestlone/secrets/secrets.json`. Treat both as plaintext sensitive
  files.
- `nestlone auth status --provider <id>` reports which secret backend is
  active for a provider.

### Self-update

`nestlone update` on Android requests `nestlone-android-arm64` and
`nestlone-tui-android-arm64` release assets — never the Linux arm64
...
| Nestlone command sandbox | ❌ unavailable | No bubblewrap/Seatbelt backend on Android |
| Nestlone keyring backend | ❌ unavailable | Falls back to file-backed secrets |
...
| Shell execution | ⚠️ app boundary only | No Nestlone-specific narrowing; runtime QA pending |
