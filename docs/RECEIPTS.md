...
runtime records that Nestlone already owns: thread metadata, turn status, turn
...
```text
nestlone receipt export --thread <thread_id> --turn <turn_id> --format json
...
`nestlone review --write-receipt` writes a local JSON receipt for the reviewed
diff under the Nestlone state directory (`review-receipts/`) unless
...
`nestlone review --write-receipt` after changing the diff; reviewers should
compare the `diff_fingerprint` before reusing a receipt in a PR handoff.

`nestlone review --check-receipt` is the local pre-push gate. It does not call
...
```json
{
  "schema_id": "nestlone.conformance-receipt/v0",
  "thread": {
    "id": "thr_...",
