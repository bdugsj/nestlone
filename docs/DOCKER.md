...
Nestlone publishes a multi-arch Linux image to GitHub Container Registry
for each release.

```bash
docker pull ghcr.io/hmbown/nestlone:latest
```
...
docker run --rm -it \
  ...
  -v "$PWD:/workspace" \
  -w /workspace \
  ghcr.io/hmbown/nestlone:vX.Y.Z
```

Replace `vX.Y.Z` with a tag from
[GitHub Releases](https://github.com/bdugsj/nestlone/releases).

## Default image contract

`ghcr.io/hmbown/nestlone:latest` and the semver tags are conservative runtime
images:

- the container runs as the non-root `nestlone` user with UID/GID `1000:1000`
- the image does not grant passwordless `sudo`
- the image is meant to run Nestlone against mounted workspaces, not to mutate
  the base operating system at runtime
- user state belongs in a volume mounted at `/home/nestlone/.nestlone`
...
```bash
docker volume create nestlone-my-project-home

docker run --rm -it \
  -e DEEPSEEK_API_KEY="$DEEPSEEK_API_KEY" \
  -v nestlone-my-project-home:/home/nestlone/.nestlone \
  -v "$PWD:/workspace" \
  -w /workspace \
  nestlone-toolbox:my-project
```

Inside this opt-in image, Nestlone can use commands such as
...
```bash
CODEWHALE_IMAGE=ghcr.io/hmbown/nestlone:vX.Y.Z \
CODEWHALE_TOOLBOX_IMAGE=nestlone-toolbox:my-project \
CODEWHALE_HOME_VOLUME=nestlone-my-project-home \
CODEWHALE_WORKSPACE="$PWD" \
docker compose -f docs/examples/compose.toolbox.yml run --rm nestlone
```

Use a different `CODEWHALE_TOOLBOX_IMAGE` and `CODEWHALE_HOME_VOLUME` for each
project that needs an independent toolchain or independent `.nestlone` state.
...
image="nestlone-toolbox:${project}"
docker volume create "nestlone-${project}-home"

docker run --rm -it \
  --name "nestlone-${project}" \
  -e DEEPSEEK_API_KEY="$DEEPSEEK_API_KEY" \
  -v "nestlone-${project}-home:/home/nestlone/.nestlone" \
  -v "$PWD:/workspace" \
  -w /workspace \
  "$image"
```
...
example `nestlone-toolbox:frontend` and `nestlone-toolbox:backend`. The
...
Nestlone does not automatically execute `.nestlone/setup.sh` or legacy
...
For example, to run a committed bootstrap script before starting Nestlone:
...
docker run --rm -it \
  ...
  --entrypoint bash \
  nestlone-toolbox:my-project \
  -lc './scripts/bootstrap-dev.sh && exec nestlone'
```
...
docker run --rm -it \
  ...
  --entrypoint bash \
  nestlone-toolbox:my-project \
  -lc 'sudo update-ca-certificates && exec nestlone'
```
...
Mount `/home/nestlone/.nestlone` to persist sessions, config, skills, memory,
and the offline queue across container restarts. The image also keeps
`/home/nestlone/.deepseek` available for legacy compatibility. A
...
non-root `nestlone` user with UID/GID `1000:1000`. The mounted directory must be
writable by that user, or startup can fail while creating runtime directories
under `.nestlone/tasks`. On Linux hosts, either use the named volume above or
prepare the bind mount explicitly:

```bash
mkdir -p ~/.nestlone
sudo chown -R 1000:1000 ~/.nestlone

docker run --rm -it \
  -e DEEPSEEK_API_KEY="$DEEPSEEK_API_KEY" \
  -v ~/.nestlone:/home/nestlone/.nestlone \
  ghcr.io/hmbown/nestlone:latest
```

That `chown` changes ownership of the host `~/.nestlone` directory. Skip it if
you do not want the container UID to own your local config, and use a named
volume instead.

## Non-interactive / pipeline usage

When stdin is not a TTY, `nestlone` drops to the dispatcher's one-shot mode
(`nestlone -c "…"`). Pipe a prompt on stdin:

```bash
echo "Explain the Cargo.toml in structured English." | \
  docker run --rm -i -e DEEPSEEK_API_KEY ghcr.io/hmbown/nestlone:latest
```
...
docker buildx build --platform linux/amd64,linux/arm64 -t nestlone .
```
...
rust-analyzer, and the `nestlone` binary. Open the repo in a devcontainer to get a
