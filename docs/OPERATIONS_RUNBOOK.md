...
1. Confirm binary + config:
   - `cargo run -- --version`
   - `cat ~/.nestlone/config.toml` (or inspect configured profile)
...
3. Capture current state:
   - `ls ~/.nestlone/sessions`
   - `ls ~/.nestlone/sessions/checkpoints`
   - `ls ~/.nestlone/tasks`
...
- Queue state persists to `~/.nestlone/sessions/checkpoints/offline_queue.json`
...
Expected behavior:
- Checkpoint stored at `~/.nestlone/sessions/checkpoints/latest.json`
- Startup begins a fresh session unless `--resume`/`--continue` is supplied

Actions:
1. Resume prior work explicitly via `nestlone --resume <id>` or `Ctrl+R` in TUI
...
Affected stores:
- sessions (`~/.nestlone/sessions/*.json`)
- runtime thread/turn/item records
- tasks (`~/.nestlone/tasks/tasks/*.json`)
...
Checks:
1. Validate `~/.nestlone/mcp.json` schema and server command paths
