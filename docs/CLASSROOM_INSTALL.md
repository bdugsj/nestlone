# Nestlone Classroom / Lab Install Checklist

A step-by-step checklist for IT admins deploying Nestlone on lab or classroom
...
| 4 | Obtain the installer: download `NestloneSetup.exe` from a v0.8.50+ [release](https://github.com/bdugsj/nestlone/releases/latest) or from your department mirror. | ☐ |
| 5 | Verify SHA-256 hash against `nestlone-artifacts-sha256.txt` before deploying. | ☐ |
...
- Installs to `%LOCALAPPDATA%\Programs\Nestlone\bin`
...
1. Double-click `NestloneSetup.exe`.
...
```powershell
# 1. Create directory
$binDir = "$env:LOCALAPPDATA\Programs\Nestlone\bin"
New-Item -ItemType Directory -Force -Path $binDir

# 2. Download binaries (adjust URL to your mirror or release tag)
$tag = (Invoke-RestMethod -Uri "https://api.github.com/repos/bdugsj/nestlone/releases/latest").tag_name
Invoke-WebRequest -Uri "https://github.com/bdugsj/nestlone/releases/download/$tag/nestlone-windows-x64.exe"     -OutFile "$binDir\nestlone.exe"
Invoke-WebRequest -Uri "https://github.com/bdugsj/nestlone/releases/download/$tag/nestlone-tui-windows-x64.exe" -OutFile "$binDir\nestlone-tui.exe"
...
|---|---------|-----------------|-------|
| 1 | `nestlone --version` | Prints version string | ☐ |
| 2 | `nestlone doctor` | All checks pass | ☐ |
| 3 | `nestlone-tui --version` | Prints version string | ☐ |

If `nestlone` is not found, the user may need to open a **new** terminal window for PATH changes to take effect.

## Lab validation checklist

Run this once on a clean lab machine, and again on a machine that already has a
previous Nestlone install:

| # | Scenario | Expected result | Done? |
|---|----------|-----------------|-------|
| 1 | Install with no existing Nestlone PATH entry | Adds exactly `%LOCALAPPDATA%\Programs\Nestlone\bin` | ☐ |
| 2 | Install twice | PATH is not duplicated | ☐ |
| 3 | Install with a neighboring PATH entry such as `C:\Tools\Nestlone\bin-extra` | Neighboring entry is preserved | ☐ |
| 4 | Upgrade by installing a newer `NestloneSetup.exe` over an older one | Apps & Features version and both `--version` outputs match the new build | ☐ |
...
Or create a `config.toml` in `%APPDATA%\nestlone\`:
...
```powershell
& "$env:LOCALAPPDATA\Programs\Nestlone\Uninstall.exe" /S
```
...
| Symptom | Fix |
|---------|-----|
| `nestlone` not found after install | Open a **new** terminal. If still missing, check PATH: `echo $env:Path` |
| `MISSING_COMPANION_BINARY` | Ensure both `nestlone.exe` and `nestlone-tui.exe` are in the same directory |
...
| `nestlone doctor` fails API check | Verify `OPENAI_API_KEY` is set or `config.toml` exists |
...
1. Install Nestlone using Option A (silent) or Option C (manual).
2. Do **not** set API keys in the image — these are per-user/per-student.
3. The install directory (`%LOCALAPPDATA%\Programs\Nestlone\bin`) is per-user,
...
4. Alternatively, install to a shared location like `C:\Tools\Nestlone\bin`
   and add it to the **machine** PATH:
   ```powershell
   [Environment]::SetEnvironmentVariable("Path", "$env:Path;C:\Tools\Nestlone\bin", "Machine")
   ```
...
|------|-----------------|
| Binaries | `%LOCALAPPDATA%\Programs\Nestlone\bin\` |
| User config | `%APPDATA%\nestlone\config.toml` |
| Uninstaller | `%LOCALAPPDATA%\Programs\Nestlone\Uninstall.exe` |
