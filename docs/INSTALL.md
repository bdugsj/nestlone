...
This branch describes the **v0.0.2 source candidate**. Install commands that use
...
It downloads the matching `nestlone`, `nest`, and `nestlone-tui` release binaries,
verifies them against `nestlone-artifacts-sha256.txt`, installs to
`~/.local/bin` by default, and exposes the `nest` convenience command.

---

## 1. Supported platforms

Published Nestlone releases ship matched `nestlone`, `nest`, and
`nestlone-tui` prebuilt binaries for their supported platform/architecture
combinations. The table below is the intended v0.0.2 candidate matrix;
...
| Android / Termux | arm64 (aarch64) | ⚠️⁴ preview | ⚠️⁴ preview | `nestlone-android-arm64.tar.gz` preview archive when published |
...
| Linux x64 on musl (Alpine) | ✅ (static) |    ✅      |       ✅        | static `nestlone-tui-linux-x64` (musl) asset           |
...
⁴ The v0.0.2 source-candidate npm wrapper recognizes Android arm64 and resolves
  the matching `nestlone`, `nest`, and `nestlone-tui` Android assets. npm
...
GNU libc `nestlone-linux-arm64` archive in Termux; use the Termux-specific
...
The v0.0.2 candidate build runs on Ubuntu 24.04, so it can require `GLIBC_2.39`.
...
older systems without error. The v0.0.2 candidate GNU arm64 asset is built on
...
The npm wrapper, `nestlone update`, and the Unix archive installer preflight
...
> from `npm i -g nestlone`. v0.8.8 publishes both `nestlone-linux-arm64`
> and `nestlone-tui-linux-arm64`, so a plain `npm i -g nestlone` works
...
When the release includes `nestlone-android-arm64.tar.gz`, install it with the
...
```bash
cd "$HOME"
curl -L -O https://github.com/bdugsj/nestlone/releases/latest/download/nestlone-android-arm64.tar.gz
curl -L -O https://github.com/bdugsj/nestlone/releases/latest/download/nestlone-bundles-sha256.txt
sha256sum -c nestlone-bundles-sha256.txt --ignore-missing

tar xzf nestlone-android-arm64.tar.gz
cd nestlone-android-arm64
...
temporary credentials. `nestlone auth set` is available, but the Termux build
has no supported OS keyring integration and falls back to file-backed secrets
by writing `~/.nestlone/config.toml` and mirroring keys to
`~/.nestlone/secrets/secrets.json`. Both are plaintext files protected by
`0600` permissions and are not encrypted at rest.

```bash
nestlone auth set --provider deepseek
nestlone auth status
nestlone doctor
```
...
```bash
command -v nestlone nest nestlone-tui
test -x "$PREFIX/bin/nestlone"
test -x "$PREFIX/bin/nest"
test -x "$PREFIX/bin/nestlone-tui"

nestlone --version
nestlone doctor
nestlone exec --auto "run pwd"
nestlone-tui --version
```

Known limitations:

- Commands inherit Android's per-app UID, SELinux, and seccomp protections and
  any permissions granted to Termux. Nestlone's opt-in bubblewrap
  child-process sandbox is Linux-only and is not built on Android, so approved
  commands receive no Nestlone-specific filesystem narrowing.
- The Termux build has no supported Android Keystore or desktop Secret Service
  integration. Use `nestlone auth status` to confirm the active source and
...
- Terminal rendering varies by Android terminal app. The TUI always owns the
  ...
  use `nestlone exec` for headless runs instead.
...
`https://github.com/bdugsj/nestlone/releases` and the npm package named
`nestlone`. Do not install release assets from look-alike repositories,
...
`nestlone-artifacts-sha256.txt` for bare binaries and
`nestlone-bundles-sha256.txt` for `.tar.gz` / `.zip` platform archives. If you
download binaries manually, verify them before running:

```bash
# Run from the directory containing the downloaded binaries.
curl -L -O https://github.com/bdugsj/nestlone/releases/latest/download/nestlone-artifacts-sha256.txt
sha256sum -c nestlone-artifacts-sha256.txt --ignore-missing
```

On macOS, use
`shasum -a 256 -c nestlone-artifacts-sha256.txt --ignore-missing` instead of
`sha256sum`.
...
- the filename, for example `nestlone-linux-x64`
...
release, verifies a SHA-256 manifest, and exposes `nestlone`, `nest`, and
`nestlone-tui` on your `PATH`.
...
| `DEEPSEEK_TUI_VERSION`              | Legacy alias for `CODEWHALE_VERSION` (defaults to `nestloneBinaryVersion`)            |
...
`nestlone-cli` installs the `nestlone` and `nest` commands, while
`nestlone-tui` installs the `nestlone-tui` command used by the dispatcher.
...
> from source, and on Linux the `nestlone-tui` crate links against
...
```sh
nix run github:bdugsj/nestlone
```

Nix builds `nestlone-tui` and then starts the `nestlone` dispatcher. Pass
arguments after `--`, for example:

```sh
nix run github:bdugsj/nestlone -- --help
```
...
```nix
{
  inputs = {
    ...
    nestlone-tui.url = "github:bdugsj/nestlone";
    nestlone-tui.inputs.nixpkgs.follows = "nixpkgs";
  };
}
```
...
```nix
{
  outputs = { self, nixpkgs, nestlone-tui }:
  let
  ...
  in
  {
    ...
    nixosConfigurations.yourhostname = nixpkgs.lib.nixosSystem {
      ...
      modules = [
        # ...
        {
          environment.systemPackages = [ nestlone-tui.packages.${system}.default ];
        }
      ];
    };
  };
...
compatibility while the formula is renamed to `nestlone`. It installs the
...
Update with `brew upgrade deepseek-tui`. There is no `nestlone` formula yet;
...
the **bare binaries** (`nestlone-<platform>`, `nest-<platform>`, and
`nestlone-tui-<platform>`, no extension) and a **`.tar.gz` / `.zip` archive**
(`nestlone-<platform>.tar.gz`) that bundles the same commands plus an
`install.sh`. The npm wrapper and the in-app `nestlone update` download the
...
> ```bash
> xattr -d com.apple.quarantine ~/.local/bin/nestlone ~/.local/bin/nest ~/.local/bin/nestlone-tui 2>/dev/null || true
> ```

Verify integrity against the per-release SHA-256 manifest:

```bash
curl -L -o /tmp/nestlone-artifacts-sha256.txt \
    https://github.com/bdugsj/nestlone/releases/latest/download/nestlone-artifacts-sha256.txt
( cd ~/.local/bin && sha256sum -c /tmp/nestlone-artifacts-sha256.txt --ignore-missing )
```

(Use `shasum -a 256 -c /tmp/nestlone-artifacts-sha256.txt --ignore-missing`
...
npm install -g nestlone@X.Y.Z

# Cargo path: two packages provide nestlone + nest + nestlone-tui
cargo install nestlone-cli --version X.Y.Z --locked --force
cargo install nestlone-tui --version X.Y.Z --locked --force
```
...
```bash
# individual binaries
curl -L -o nestlone-artifacts-sha256.txt \
  https://github.com/bdugsj/nestlone/releases/download/vX.Y.Z/nestlone-artifacts-sha256.txt

# platform archives
curl -L -o nestlone-bundles-sha256.txt \
  https://github.com/bdugsj/nestlone/releases/download/vX.Y.Z/nestlone-bundles-sha256.txt
```

Inside a Nestlone workspace, `/restore list [N]` lists side-git file snapshots
...
The `nestlone` package is listed in Scoop's main bucket:

```powershell
scoop update
scoop install nestlone
nestlone --version
```
...
`nestlone-windows-arm64.zip` from the same release; both paths then use native
ARM64 binaries.

**Download** `NestloneSetup.exe` from the
[Releases page](https://github.com/bdugsj/nestlone/releases/latest).

**Install** by double-clicking the setup executable. The installer:

- Installs `nestlone.exe`, `nest.exe`, and `nestlone-tui.exe` side-by-side into
  `%LOCALAPPDATA%\Programs\Nestlone\bin`
...
for each user profile that needs Nestlone.

The release-built installer is currently unsigned and may trigger Windows
SmartScreen. Verify the SHA-256 checksum from `nestlone-artifacts-sha256.txt`
...
# Place nestlone.exe, nest.exe, and nestlone-tui.exe here, then:
makensis /DVERSION=<version> nestlone.nsi
```
...
cargo install --path crates/cli --locked   # provides `nestlone` and `nest`
cargo install --path crates/tui --locked   # provides `nestlone-tui`
...
# Per build
cross build --release --target aarch64-unknown-linux-gnu -p nestlone-cli
cross build --release --target aarch64-unknown-linux-gnu -p nestlone-tui
```

The resulting binaries land in
`target/aarch64-unknown-linux-gnu/release/nestlone` and
`target/aarch64-unknown-linux-gnu/release/nestlone-tui`. Copy the matched pair
...
cargo build --release --target aarch64-unknown-linux-gnu -p nestlone-cli
cargo build --release --target aarch64-unknown-linux-gnu -p nestlone-tui
```
...
The binaries appear in `target\release\nestlone.exe`,
`target\release\nest.exe`, and `target\release\nestlone-tui.exe`.
...
binaries. Either upgrade (`npm i -g nestlone@latest`) or use
`cargo install` per [Section 4](#4-install-via-cargo-any-tier-1-rust-target).

### `MISSING_COMPANION_BINARY` at runtime

The dispatcher (`nestlone`) requires the TUI runtime (`nestlone-tui`) to be on
...
### `nestlone update` reports `no asset found for platform nestlone-linux-aarch64`
...
### `nestlone update` is blocked by GitHub from mainland China

`nestlone update` normally contacts GitHub Releases for metadata and binary
...
they provide the `nestlone`, `nest`, and `nestlone-tui` commands:

To check the latest release without downloading or replacing binaries, run
`nestlone update --check`.

```bash
cargo install --git https://cnb.cool/codewhale.net/nestlone --tag vX.Y.Z nestlone-cli --locked --force
cargo install --git https://cnb.cool/codewhale.net/nestlone --tag vX.Y.Z nestlone-tui     --locked --force
```

If you operate a binary asset mirror, `nestlone update` can use it directly:

```bash
CODEWHALE_RELEASE_BASE_URL=https://your-mirror.example.com/Nestlone/vX.Y.Z/ \
DEEPSEEK_TUI_VERSION=X.Y.Z \
nestlone update
```

The mirror directory must contain `nestlone-artifacts-sha256.txt` and the
...
Together they provide `nestlone`, `nest`, and `nestlone-tui`. For
...
nestlone-tui --locked` fails while compiling the keyring or D-Bus secret
...
`nestlone`, `nest`, and `nestlone-tui`:
...
only apply when WSL2 is compiling Nestlone from source.

### Wrapper installs but `nestlone` isn't found
...
4. **Use `cargo install nestlone-cli --locked`** from crates.io — this
...
If `nestlone` waits several seconds and prints `connect ETIMEDOUT` or
...
2. Mirror the release assets internally and set `DEEPSEEK_TUI_RELEASE_BASE_URL`:
   ...
   ```bash
   export DEEPSEEK_TUI_RELEASE_BASE_URL=https://your-mirror.example.com/Nestlone/
   nestlone
   ```

   The directory must contain `nestlone-artifacts-sha256.txt` and the platform
...
4. Download all three matching `nestlone`, `nest`, and `nestlone-tui`
   binaries from the [Releases page](https://github.com/bdugsj/nestlone/releases),
...
```bash
nestlone --version
nestlone doctor       # checks API key, provider, runtime, and PATH integrity
