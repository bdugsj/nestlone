...
Nestlone will not run an arbitrary user script between a model response and
...
- `nestlone exec --output-format text|stream-json` selects a documented output
  encoding. Structured output retains block identity so downstream tools can
  select `thinking` or `text` without Nestlone rewriting either.
...
`stream-json` and apply its own bounded transform outside Nestlone. Its failure
...
```sh
nestlone exec --output-format stream-json "..." \
...
use case while keeping Nestlone's canonical receipts trustworthy.
