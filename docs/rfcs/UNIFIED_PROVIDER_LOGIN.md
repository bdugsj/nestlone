# RFC: Unified provider login (`nestlone auth login`)
...
```
nestlone auth login --provider <anthropic|openai-codex|xai|...>
/auth <provider>
```

Today the tree has one interactive flow — xAI device-code OAuth
(`crates/tui/src/xai_oauth.rs`, `nestlone auth xai-device`, `/auth
...
Nestlone is permitted to use that flow. Those details may be
