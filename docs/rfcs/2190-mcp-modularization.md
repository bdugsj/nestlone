...
### 1.1 `nestlone-mcp` crate (`crates/mcp/`)
...
- **MCP stdio server** — a minimal MCP server that exposes Nestlone's own tools
  over stdio for external MCP clients. Used by the `nestlone mcp` CLI subcommand.
...
- The `nestlone` CLI (non-interactive mode)
...
**Dependencies:** `serde`, `serde_json`, `nestlone-protocol` (for tool schema)
...
2. Re-export from `nestlone-mcp` for backward compatibility
3. Update `Cargo.toml` in `nestlone-mcp` to depend on `nestlone-mcp-protocol`
...
3. Update `nestlone-tui` to depend on `nestlone-mcp-client`
4. Update `nestlone-cli` to depend on `nestlone-mcp-server`
5. Deprecate `nestlone-mcp` crate (re-exports from new crates)
...
2. Encrypted file in `~/.nestlone/mcp-credentials/` (fallback)
...
| Breaking internal imports | Phase 2 carries `nestlone-mcp` deprecation shim for one release |
