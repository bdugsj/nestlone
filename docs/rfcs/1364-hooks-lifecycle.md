...
this RFC fires from `nestlone exec`, the CLI subcommands, the app-server /
ACP surfaces, or the `workflow` tool. The RFC's use of "Nestlone" as the
actor should be read as "the Nestlone TUI" throughout.
...
Nestlone already has lifecycle hooks and MCP support, but the current hook
