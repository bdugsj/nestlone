...
For v0.9.0, Nestlone should treat harness profiles as typed policy data first.
...
Only after those surfaces are visible and tested should Nestlone add evidence
...
`.nestlone/constitution.json` is local repo law, not another provider profile.
...
- the repo-law source, such as `.nestlone/constitution.json` or none.
