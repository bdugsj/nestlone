...
description: "Close resolved Nestlone issues only after verifying the landed commit/behavior, with a positive crediting comment; never from title alone."
---
...
1. **Read the issue from the source, not the title.** Pull the body, labels,
   ...
   ```bash
   gh issue view N --repo bdugsj/nestlone \
...
5. **Close with that comment in one step** (only with maintainer approval where
   policy requires it):
   ```bash
   gh issue close N --repo bdugsj/nestlone -r completed \
...
keep it open:
```bash
gh issue comment N --repo bdugsj/nestlone \
