# Nestlone Architecture

This document provides an overview of the nestlone architecture for developers and contributors.
...
- **`crates/lane`** - Lane runtime: durable, attachable running instances of
  Fleet/Workflow work (`nestlone lane list/status/attach/logs/stop`).
...
- **`runtime_api.rs`** - HTTP/SSE runtime API (`nestlone serve --http`)
...
1. Before sending user input, the TUI writes a checkpoint snapshot to `~/.nestlone/sessions/checkpoints/latest.json`
2. Startup remains fresh by default; prior sessions are resumed explicitly via `--resume`/`--continue` (or `Ctrl+R` in TUI)
3. While degraded/offline, new prompts are queued in-memory and mirrored to `~/.nestlone/sessions/checkpoints/offline_queue.json`
...
6. Action-capable turns also take pre/post-turn side-git workspace snapshots under `~/.nestlone/snapshots/<project_hash>/<worktree_hash>/.git`; `/restore N` and `revert_turn` restore file state without changing conversation history or the user's `.git`
...
2. `task_manager.rs` persists task + queue entry under `~/.nestlone/tasks`
...
1. Configure in `~/.nestlone/mcp.json`
...
3. Place in a Nestlone-owned root (`~/.nestlone/skills/` or
   `<workspace>/.nestlone/skills/`), or import from a compatible harness root
   through `/skills`
...
Configure in `~/.nestlone/config.toml`:
...
- `~/.nestlone/config.toml` - Main configuration (`~/.deepseek/config.toml` is still read as a legacy fallback)
...
- `~/.nestlone/mcp.json` - MCP server configuration
- `~/.nestlone/skills/` - User skills directory
- `~/.nestlone/sessions/` - Session history
- `~/.nestlone/sessions/checkpoints/` - Crash checkpoint + offline queue persistence
- `~/.nestlone/snapshots/` - Side-git pre/post-turn workspace snapshots for `/restore` and `revert_turn`
- `~/.nestlone/tasks/` - Background task records, queue, timelines, artifacts
- `~/.nestlone/audit.log` - Append-only audit events for credential + approval/elevation actions
