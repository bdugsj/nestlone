  ...
  assert.throws(
    () =>
      assertReleaseAssetsAbsent(
        {
          assets: [
            { name: "nestlone-linux-x64" },
            { name: "nestlone-artifacts-sha256.txt" },
          ],
        },
        "v0.9.1",
      ),
    /Refusing to replace existing assets for v0\.9\.1: nestlone-linux-x64, nestlone-artifacts-sha256\.txt/,
  );
});
...
test("release lookup treats only an explicit GitHub 404 as absent", () => {
  ...
  assert.equal(
    fetchRelease("bdugsj/nestlone", "v0.9.1", "gh", () => {
      throw notFound;
    }),
    null,
  );
...
test("release lookup parses the exact tag endpoint and validates targets", () => {
  let invocation;
  const release = fetchRelease("bdugsj/nestlone", "v0.9.1", "/fake/gh", (...args) => {
    invocation = args;
    return JSON.stringify({ assets: [] });
  });
  assert.deepEqual(release, { assets: [] });
  assert.equal(invocation[0], "/fake/gh");
  assert.deepEqual(invocation[1], ["api", "repos/bdugsj/nestlone/releases/tags/v0.9.1"]);
  assert.doesNotThrow(() => validateTarget("bdugsj/nestlone", "v0.9.1"));
  assert.throws(() => validateTarget("bad repo", "v0.9.1"), /Invalid GitHub repository/);
  assert.throws(() => validateTarget("bdugsj/nestlone", "main"), /Invalid release tag/);
});

test("release lookup rejects malformed successful API output", () => {
  assert.throws(
    () => fetchRelease("bdugsj/nestlone", "v0.9.1", "gh", () => "not-json"),
