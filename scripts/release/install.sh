#!/usr/bin/env bash
set -euo pipefail
# Nestlone Unix installer
# Copies nestlone, nest, and nestlone-tui to ~/.local/bin (or $PREFIX/bin)
...
preflight_glibc() {
    ...
    if [[ "${NESTLONE_SKIP_GLIBC_CHECK:-}" == "1" || "${CODEWHALE_SKIP_GLIBC_CHECK:-}" == "1" || "${DEEPSEEK_TUI_SKIP_GLIBC_CHECK:-}" == "1" || "${DEEPSEEK_SKIP_GLIBC_CHECK:-}" == "1" ]]; then
        return 0
    fi
    ...
    if ! host="$(detect_host_glibc)" || [[ -z "$host" ]]; then
        ...
        echo "Build from source instead: cargo install nestlone-cli --locked && cargo install nestlone-tui --locked" >&2
        echo "Set NESTLONE_SKIP_GLIBC_CHECK=1 to bypass this check at your own risk." >&2
        return 1
    fi
...
for bin in nestlone nest nestlone-tui; do
    src="$SCRIPT_DIR/$bin"
    dst="$BIN_DIR/$bin"
    if [[ ! -f "$src" ]]; then
