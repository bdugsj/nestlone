...
# only asks Cargo to resolve the nestlone-tui dependency graph for the OHOS
...
# `bindgen` feature for nestlone-workflow-js, which is the only reason the
...
package="${NESTLONE_OHOS_DEP_PACKAGE:-${CODEWHALE_OHOS_DEP_PACKAGE:-nestlone-tui}}"
workflow_js_package="${NESTLONE_OHOS_WORKFLOW_JS_PACKAGE:-${CODEWHALE_OHOS_WORKFLOW_JS_PACKAGE:-nestlone-workflow-js}}"
...
cargo_tree_with_retry() {
  ...
  shift
  local attempt
  local max_attempts="${NESTLONE_OHOS_DEP_RETRIES:-${CODEWHALE_OHOS_DEP_RETRIES:-3}}"
  local delay_seconds="${NESTLONE_OHOS_DEP_RETRY_DELAY_SECONDS:-${CODEWHALE_OHOS_DEP_RETRY_DELAY_SECONDS:-10}}"
  local err_file
  local output
  local status

  if ! [[ "${max_attempts}" =~ ^[0-9]+$ ]] || ((max_attempts < 1)); then
    echo "NESTLONE_OHOS_DEP_RETRIES must be an integer greater than or equal to 1." >&2
    return 1
  fi
...
# nestlone-workflow-js only compiles for OHOS because its
