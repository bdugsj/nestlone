...
if npm_version="$(npm view "nestlone@${version}" version 2>/dev/null)"; then
  echo "npm nestlone@${npm_version} is published."
else
  echo "npm nestlone@${version} is not published." >&2
  fail=1
fi

# `nestloneBinaryVersion` is the internal version-pin field. Fall back
# to the legacy `deepseekBinaryVersion` field for old/transition packages.
binary_field=""
npm_binary_version=""
if value="$(npm view "nestlone@${version}" nestloneBinaryVersion 2>/dev/null)" && [[ -n "${value}" ]]; then
  binary_field="nestloneBinaryVersion"
  npm_binary_version="${value}"
elif value="$(npm view "nestlone@${version}" deepseekBinaryVersion 2>/dev/null)" && [[ -n "${value}" ]]; then
...
if [[ -n "${binary_field}" ]]; then
  ...
  echo "npm nestloneBinaryVersion is absent (allowed packaging-only mismatch)."
else
  echo "npm nestloneBinaryVersion is absent for nestlone@${version}." >&2
  fail=1
fi
...
crates_user_agent="Nestlone release check (https://github.com/bdugsj/nestlone)"
...
if [[ "${fail}" == "0" ]]; then
  echo "Published release OK: npm nestlone@${version} and ${#release_crates[@]} crates are visible."
fi
