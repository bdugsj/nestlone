...
#
#   1. app-server stdio probe (always): drives `nestlone app-server --stdio`
...
#      from `nestlone auth list`, maps each to a cheap sentinel model, and
...
#   scripts/release/app-server-smoke.sh --bin ./target/release/nestlone
#
# Binary resolution order: --bin <path>, $NESTLONE_BIN (legacy $CODEWHALE_BIN), ./target/release/nestlone, PATH.
...
PASS=0
FAIL=0
BIN="${NESTLONE_BIN:-${CODEWHALE_BIN:-}}"
DO_MATRIX=0
DRY_RUN=1
...
resolve_bin() {
    if [[ -n "$BIN" ]]; then
        [[ -x "$BIN" ]] || { echo "nestlone binary not executable: $BIN" >&2; exit 2; }
        return
    fi
    if [[ -x "./target/release/nestlone" ]]; then
        BIN="./target/release/nestlone"
    elif command -v nestlone >/dev/null 2>&1; then
        BIN="$(command -v nestlone)"
    else
        echo "could not find a nestlone binary." >&2
        echo "  build one: cargo build -p nestlone-cli --release" >&2
        echo "  or pass:   --bin <path> / NESTLONE_BIN=<path>" >&2
        exit 2
    fi
}
...
# Echo configured provider slugs (active != missing) from `nestlone auth list`.
