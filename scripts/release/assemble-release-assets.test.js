...
function makeIntermediateArtifacts(root) {
  const generated = new Set(["nestlone.bat", CHECKSUM_MANIFEST]);
  ...
  for (const name of copied) {
    ...
    const artifactDirectory = BUNDLE_ASSET_NAMES.includes(name)
      ? path.join(root, "nestlone-bundles")
      : path.join(root, name);
    fs.mkdirSync(artifactDirectory, { recursive: true });
    fs.writeFileSync(path.join(artifactDirectory, name), `fixture:${name}\n`);
  }

  const bundleManifestDirectory = path.join(root, "nestlone-bundles");
...
test("assembly creates and verifies the exact release asset directory", async () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nestlone-asset-assembly-"));
  ...
  try {
    ...
    assert.equal(
      fs.readFileSync(path.join(output, "nestlone.bat"), "utf8"),
      windowsLauncherContents(),
    );
...
test("bundle helper creates the exact nine archives and checksum manifest", () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nestlone-bundle-assembly-"));
  const input = path.join(tempRoot, "input");
  const output = path.join(tempRoot, "output");
  ...
  try {
    ...
    for (const name of allReleaseAssetNames().filter((asset) =>
      /^(nestlone|nest|nestlone-tui)-(linux|android|macos|windows)-/.test(asset) &&
    ...
    const linuxEntries = execFileSync(
      "tar",
      ["-tzf", path.join(output, "nestlone-linux-x64.tar.gz")],
      { encoding: "utf8" },
    );
    for (const entry of ["nestlone", "nest", "nestlone-tui", "install.sh"]) {
      assert.match(linuxEntries, new RegExp(`nestlone-linux-x64/${entry}\\n`));
    }
    const extracted = path.join(tempRoot, "extracted");
    fs.mkdirSync(extracted);
    execFileSync(
      "tar",
      ["-xzf", path.join(output, "nestlone-linux-x64.tar.gz"), "-C", extracted],
      { stdio: "pipe" },
    );
    for (const entry of ["nestlone", "nest", "nestlone-tui", "install.sh"]) {
      const mode = fs.statSync(path.join(extracted, "nestlone-linux-x64", entry)).mode & 0o777;
      assert.equal(mode, 0o755, `${entry} should remain executable after artifact transport`);
    }
    const portableEntries = execFileSync(
      "unzip",
      ["-Z1", path.join(output, "nestlone-windows-arm64-portable.zip")],
      { encoding: "utf8" },
    );
    assert.match(portableEntries, /nestlone-windows-arm64-portable\/nest\.exe\n/);
...
test("verification rejects modified and unexpected assets", async () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nestlone-asset-tamper-"));
  const input = path.join(tempRoot, "input");
  const output = path.join(tempRoot, "output");
  try {
    fs.mkdirSync(input, { recursive: true });
    makeIntermediateArtifacts(input);
    await assemble(input, output);
    fs.appendFileSync(path.join(output, "nestlone-linux-x64"), "tampered\n");
    await assert.rejects(
      () => verifyAssetDirectory(output),
      /checksum mismatch for nestlone-linux-x64/,
    );
