...
if [[ "${binary_version}" != "${version}" ]]; then
  if [[ "${NESTLONE_ALLOW_NPM_BINARY_MISMATCH:-0}" == "1" || "${CODEWHALE_ALLOW_NPM_BINARY_MISMATCH:-0}" == "1" ]]; then
    ...
    echo "Set NESTLONE_ALLOW_NPM_BINARY_MISMATCH=1 only for an intentional packaging-only npm release." >&2
    exit 1
  fi
fi
