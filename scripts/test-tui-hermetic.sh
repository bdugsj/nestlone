#!/bin/sh
# Run the TUI bin tests without reading or writing the developer's Nestlone
...
while [ "$run" -le "$runs" ]; do
  root=$(mktemp -d "${TMPDIR:-/tmp}/nestlone-tui-test.XXXXXX")
  cleanup_roots="$cleanup_roots $root"
  mkdir -p "$root/home" "$root/nestlone-home" "$root/xdg"
  ...
  (
    ...
    CODEWHALE_HOME="$root/nestlone-home" \
    XDG_CONFIG_HOME="$root/xdg" \
    DEEPSEEK_CONFIG_PATH="$root/nestlone-home/config.toml" \
    ...
    PATH="$toolchain_bin:$PATH" \
      sh -c '
        if [ -n "$1" ]; then
          exec "$2" test --quiet -p nestlone-tui --bin nestlone-tui --locked "$1"
        fi
        exec "$2" test --quiet -p nestlone-tui --bin nestlone-tui --locked
