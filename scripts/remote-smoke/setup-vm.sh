#!/usr/bin/env bash
# EXPERIMENTAL — one-shot Nestlone + Telegram bridge setup for a fresh
...
# The file is shredded after the values land in /etc/nestlone/*.env.
#
...
RELEASE_TAG="${RELEASE_TAG:-v0.0.2}"
...
if [[ ! -d /tmp/nestlone/.git ]]; then
  git clone --depth 1 --branch "$REPO_BRANCH" "$REPO_URL" /tmp/nestlone
fi

echo "== [2/8] bootstrap (user, dirs, packages, ufw, env skeletons) =="
CODEWHALE_REPO_URL="$REPO_URL" CODEWHALE_REPO_BRANCH="$REPO_BRANCH" \
  bash /tmp/nestlone/scripts/tencent-lighthouse/bootstrap-ubuntu.sh

echo "== [3/8] install prebuilt ${RELEASE_TAG} binaries (no Rust build) =="
# The systemd unit hardcodes /home/nestlone/.cargo/bin/nestlone, so we put
# the release binaries exactly there.
BIN_DIR=/home/nestlone/.cargo/bin
install -d -o nestlone -g nestlone "$BIN_DIR"
BASE="https://github.com/Hmbown/CodeWhale/releases/download/${RELEASE_TAG}"
TMP=$(mktemp -d)
curl -fsSL -o "$TMP/nestlone" "$BASE/nestlone-linux-x64"
curl -fsSL -o "$TMP/nestlone-tui" "$BASE/nestlone-tui-linux-x64"
curl -fsSL -o "$TMP/sha256.txt" "$BASE/nestlone-artifacts-sha256.txt"
( cd "$TMP"
  grep -E ' (nestlone|nestlone-tui)-linux-x64$' sha256.txt \
    | sed 's/nestlone-linux-x64/nestlone/; s/nestlone-tui-linux-x64/nestlone-tui/' \
    | sha256sum -c - )
install -m 0755 -o nestlone -g nestlone "$TMP/nestlone" "$BIN_DIR/nestlone"
install -m 0755 -o nestlone -g nestlone "$TMP/nestlone-tui" "$BIN_DIR/nestlone-tui"
rm -rf "$TMP"
sudo -u nestlone "$BIN_DIR/nestlone" --version
sudo -u nestlone "$BIN_DIR/nestlone-tui" --version

echo "== [4/8] install services (telegram bridge) =="
CODEWHALE_BRIDGE=telegram bash /tmp/nestlone/scripts/tencent-lighthouse/install-services.sh

echo "== [5/8] write secrets into /etc/nestlone/*.env =="
...
if [[ -n "$TELEGRAM_CHAT_ALLOWLIST" ]]; then
  ...
  echo "[warn] then: systemctl restart nestlone-telegram-bridge"
  set_kv "$BRIDGE_ENV" TELEGRAM_ALLOW_UNLISTED true
fi
chmod 0640 "$RUNTIME_ENV" "$BRIDGE_ENV"
chown root:nestlone "$RUNTIME_ENV" "$BRIDGE_ENV"
...
install -d -o nestlone -g nestlone -m 0700 \
  /home/nestlone/.nestlone /home/nestlone/.deepseek

echo "== [7/8] validate config =="
sudo -u nestlone node /opt/nestlone/telegram-bridge/scripts/validate-config.mjs \
  --env "$BRIDGE_ENV" --runtime-env "$RUNTIME_ENV" \
  --workspace-root /opt/nestlone --check-filesystem

echo "== [8/8] start + doctor =="
systemctl start nestlone-runtime
for _ in $(seq 1 20); do
  curl -fsS --max-time 2 http://127.0.0.1:7878/health >/dev/null 2>&1 && break
  sleep 1
done
curl -fsS --max-time 3 http://127.0.0.1:7878/health; echo
systemctl start nestlone-telegram-bridge
sleep 3
CODEWHALE_BRIDGE=telegram bash /tmp/nestlone/scripts/tencent-lighthouse/doctor.sh

echo
echo "== Setup complete. Phone smoke checklist (docs/REMOTE_VM_US.md): =="
echo "  1. DM the bot: /status"
echo "  2. /menu (tappable controls)"
echo "  3. prompt: summarize git status in /opt/nestlone/nestlone"
...
echo "  7. sudo reboot; confirm both services return: systemctl status nestlone-runtime nestlone-telegram-bridge"
