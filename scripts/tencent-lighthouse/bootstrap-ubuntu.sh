...
NESTLONE_EXTRA_REPOS="${NESTLONE_EXTRA_REPOS:-}"
...
install -d -o "${CODEWHALE_USER}" -g "${CODEWHALE_USER}" "${NESTLONE_ROOT}"
install -d -o "${CODEWHALE_USER}" -g "${CODEWHALE_USER}" "${NESTLONE_ROOT}/worktrees"
install -d -m 0750 -o root -g "${CODEWHALE_USER}" /etc/nestlone
install -d -m 0700 -o "${CODEWHALE_USER}" -g "${CODEWHALE_USER}" /var/lib/nestlone-feishu-bridge
install -d -m 0700 -o "${CODEWHALE_USER}" -g "${CODEWHALE_USER}" /var/lib/nestlone-telegram-bridge

if [[ ! -d "${NESTLONE_ROOT}/nestlone/.git" ]]; then
  sudo -u "${CODEWHALE_USER}" git clone --branch "${REPO_BRANCH}" "${REPO_URL}" "${NESTLONE_ROOT}/nestlone"
fi

for repo_spec in ${NESTLONE_EXTRA_REPOS}; do
  repo_name="${repo_spec%%=*}"
  repo_url="${repo_spec#*=}"
  if [[ -z "${repo_name}" || -z "${repo_url}" || "${repo_name}" == "${repo_url}" ]]; then
    echo "Skipping malformed NESTLONE_EXTRA_REPOS entry: ${repo_spec}" >&2
    continue
  fi
  if [[ ! -d "${NESTLONE_ROOT}/${repo_name}/.git" ]]; then
    sudo -u "${CODEWHALE_USER}" git clone "${repo_url}" "${NESTLONE_ROOT}/${repo_name}" || {
      echo "Warning: failed to clone optional repo ${repo_name} from ${repo_url}" >&2
    }
  fi
done

if [[ ! -f /etc/nestlone/runtime.env ]]; then
  cat >/etc/nestlone/runtime.env <<'EOF'
...
RUST_LOG=info
EOF
  chown root:"${CODEWHALE_USER}" /etc/nestlone/runtime.env
  chmod 0640 /etc/nestlone/runtime.env
fi

if [[ ! -f /etc/nestlone/feishu-bridge.env ]]; then
  cat >/etc/nestlone/feishu-bridge.env <<'EOF'
...
EOF
  chown root:"${CODEWHALE_USER}" /etc/nestlone/feishu-bridge.env
  chmod 0640 /etc/nestlone/feishu-bridge.env
fi
