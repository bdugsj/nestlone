# Nestlone for VS Code

Official Nestlone extension scaffold for local development.

This first slice is intentionally small:

- open Nestlone in an integrated terminal
- start `nestlone serve --http` in a visible terminal
...
npm run compile
npm run package
code --install-extension nestlone-vscode-0.8.53.vsix
```

Configure `nestlone.commandPath`, `nestlone.runtimeHost`,
`nestlone.runtimePort`, `nestlone.runtimeToken`, and
`nestlone.agentViewRefreshIntervalSeconds` from VS Code settings.
