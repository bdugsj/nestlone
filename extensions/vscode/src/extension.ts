...
export function activate(context: vscode.ExtensionContext): void {
  const output = vscode.window.createOutputChannel("Nestlone");
  const status = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 100);
  const statusView = new RuntimeStatusView();
  let autoRefreshTimer: ReturnType<typeof setInterval> | undefined;
  let autoRefreshInFlight = false;

  status.command = "nestlone.checkRuntime";
  ...
  const checkAndRefreshRuntime = async (
    ...
    const config = readRuntimeConfig();
    if (showSpinner) {
      updateStatus("$(sync~spin) Nestlone", "Checking Nestlone runtime...");
    }
    ...
    switch (state.kind) {
      case "connected":
        updateStatus("$(check) Nestlone", state.detail);
        await refreshAgentViewDetails(false);
        break;
      case "auth-required":
        updateStatus("$(lock) Nestlone", state.detail);
      ...
      case "offline":
      case "error":
        updateStatus("$(warning) Nestlone", state.detail);
  ...
  updateStatus("$(terminal) Nestlone", "Check Nestlone runtime");
  scheduleAutoRefresh();
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration((event) => {
      if (event.affectsConfiguration("nestlone.agentViewRefreshIntervalSeconds")) {
        scheduleAutoRefresh();
      }
    }),
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("nestlone.openTerminal", () => {
      const config = readRuntimeConfig();
      openNestloneTerminal(config);
      output.appendLine(`Opened Nestlone terminal using ${config.commandPath}.`);
    }),
  );

  context.subscriptions.push(
    vscode.commands.registerCommand("nestlone.startRuntime", () => {
      const config = readRuntimeConfig();
      startRuntimeTerminal(config);
      const baseUrl = runtimeBaseUrl(config);
      updateStatus("$(sync~spin) Nestlone", `Runtime terminal started for ${baseUrl}`);
      output.appendLine(`Started Nestlone runtime terminal at ${baseUrl}.`);
      void vscode.window.showInformationMessage(`Nestlone runtime starting at ${baseUrl}`);
    }),
  );

  context.subscriptions.push(
    vscode.commands.registerCommand("nestlone.checkRuntime", async () => {
      return await checkAndRefreshRuntime(true, true);
    }),
  );

  context.subscriptions.push(
    vscode.commands.registerCommand("nestlone.refreshAgentView", async () => {
      await refreshAgentViewDetails(true);
    }),
  );

  context.subscriptions.push(
    vscode.commands.registerCommand("nestlone.refreshSnapshots", async () => {
      try {
        await refreshSnapshots();
      } catch (error: unknown) {
        const detail = error instanceof Error ? error.message : String(error);
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("nestlone.openRuntimeDocs", () => {
      void vscode.env.openExternal(
        vscode.Uri.parse(
          "https://github.com/bdugsj/nestlone/blob/main/docs/RUNTIME_API.md",
        ),
      );
    }),
  );

  void vscode.commands.executeCommand("nestlone.checkRuntime");
}
