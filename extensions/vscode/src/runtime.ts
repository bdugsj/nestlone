...
export function readRuntimeConfig(): RuntimeConfig {
  const config = vscode.workspace.getConfiguration("nestlone");
  const commandPath = config.get<string>("commandPath", "nestlone").trim() || "nestlone";
...
export async function checkRuntime(config: RuntimeConfig): Promise<RuntimeState> {
  ...
  return {
    kind: "connected",
    baseUrl,
    detail: version ? `Connected to Nestlone ${version}.` : "Connected to Nestlone runtime.",
    version,
  };
}
...
export function startRuntimeTerminal(config: RuntimeConfig): vscode.Terminal {
  const terminal = vscode.window.createTerminal("Nestlone Runtime");
  const args = [
    "serve",
    "--http",
    "--host",
...
export function openNestloneTerminal(config: RuntimeConfig): vscode.Terminal {
  const terminal = vscode.window.createTerminal("Nestlone");
