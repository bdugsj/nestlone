...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildPageMetadata({
    ...
    locale,
    title: isZh ? "排障 · Nestlone 文档" : "Troubleshooting · Nestlone Docs",
    description: isZh
  ...
  const incidents = isZh
    ? [
        ...
        },
        {
          name: "网络中断 / 离线行为",
          detail:
            "离线时新提示词会排队，队列持久化在 ~/.nestlone/sessions/checkpoints/offline_queue.json。用 /queue list 查看，恢复连接后重新发送（/queue edit <n> 加回车，或走正常输入流程），队列清空后文件随之清除。",
        },
        {
          name: "崩溃恢复",
          detail:
            "检查点保存在 ~/.nestlone/sessions/checkpoints/latest.json；除非传入 --resume/--continue，启动会开新会话。用 nestlone --resume <id> 或 TUI 里的 Ctrl+R 显式恢复；若检查点 schema 比二进制新，升级二进制或移除过期检查点。",
        },
        {
          name: "持久状态 schema 错误",
          detail:
        ...
        },
        {
          name: "MCP / 工具执行失败",
          detail:
            "校验 ~/.nestlone/mcp.json 的 schema 和服务器命令路径，手动确认服务器进程能启动，并在 TUI 历史/日志中检查沙箱拒绝。用 /mcp validate 诊断，可暂时禁用出问题的服务器隔离原因，验证后再启用。",
        },
      ]
    : [
        {
        ...
        },
        {
          ...
          detail:
            "New prompts queue while offline, persisted to ~/.nestlone/sessions/checkpoints/offline_queue.json. Inspect with /queue list, restore connectivity, then re-send queued entries (/queue edit <n> plus Enter, or the normal input flow); the queue file clears when the queue empties.",
        },
        {
          name: "Crash recovery",
          detail:
            "The checkpoint lives at ~/.nestlone/sessions/checkpoints/latest.json; startup begins a fresh session unless --resume/--continue is supplied. Resume explicitly with nestlone --resume <id> or Ctrl+R in the TUI; if the checkpoint schema is newer than the binary supports, upgrade the binary or remove the stale checkpoint.",
        },
        {
        ...
        },
        {
          ...
          detail:
            "Validate the ~/.nestlone/mcp.json schema and server command paths, confirm the server process starts manually, and check sandbox denials in TUI history/logs. Use /mcp validate for diagnostics, temporarily disable a failing server to isolate the issue, and re-enable after verification.",
        },
      ];
    ...
    <section className="space-y-10">
      <section id="overview" className="scroll-mt-32">
        ...
        <p className={`${bodyClass} mt-3`}>
          {isZh
            ? "先快速分诊：确认二进制与配置（nestlone --version、~/.nestlone/config.toml），需要更详细日志时用 RUST_LOG=nestlone_cli=debug 启动（HTTP 重试/重连用 RUST_LOG=deepseek_cli::client=debug），并看一眼 ~/.nestlone/sessions 与 ~/.nestlone/tasks 的当前状态。"
            : "Start with quick triage: confirm the binary and config (nestlone --version, ~/.nestlone/config.toml), enable verbose logs with RUST_LOG=nestlone_cli=debug when needed (RUST_LOG=deepseek_cli::client=debug for HTTP retries/reconnects), and capture the current state of ~/.nestlone/sessions and ~/.nestlone/tasks."}
        </p>
      ...
      <section id="docker" className="scroll-mt-32">
        ...
        <p className={`${bodyClass} mt-3`}>
          {isZh
            ? "每个发布都会向 GitHub Container Registry 推送多架构 Linux 镜像。默认镜像是保守的运行时镜像：以非 root 的 nestlone 用户（UID/GID 1000:1000）运行，不授予免密 sudo，用户状态放在挂载到 /home/nestlone/.nestlone 的卷里。可复现的安装请固定发布标签而不是 latest。"
            : "Each release publishes a multi-arch Linux image to GitHub Container Registry. The default image is a conservative runtime image: it runs as the non-root nestlone user (UID/GID 1000:1000), grants no passwordless sudo, and keeps user state in a volume mounted at /home/nestlone/.nestlone. Pin a release tag instead of latest for reproducible installs."}
        </p>
        <pre className="code-block mt-4">{`docker volume create nestlone-home
...
docker run --rm -it \\
  ...
  -v "$PWD:/workspace" \\
  -w /workspace \\
  ghcr.io/hmbown/nestlone:latest`}</pre>
        <p className={`${bodyClass} mt-3`}>
          {isZh
