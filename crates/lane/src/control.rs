...
//!
//! Why this lives in `nestlone-lane`: it is the lowest crate that both the
//! thin `nestlone` CLI facade and the TUI (slash commands, hotbar, and the
//! `nestlone fleet …` entrypoints that the facade delegates to) already
...
pub enum ControlSurface {
    /// `nestlone …` (and the `nestlone-tui …` entrypoints it delegates to).
    Cli,
...
impl ControlOperation {
    ...
    /// dispatch of the same command, and `nestlone lane interrupt` cannot
        ...
        match self.persistence {
            PersistenceScope::LaneRegistry if !ctx.lane_registry_present => {
                Availability::unavailable(
                    ...
                    "no Lane registry yet; start one with `nestlone lane start`",
                )
            }
...
const LANE_RESTART_HINT: &str = "Lane restart has no backend: a Lane is one running Workflow and is re-created by \
     `nestlone lane start` / `nestlone workflow run`, not restarted in place.";
...
const FLEET_RESTART_HINT: &str = "Fleet restart re-leases a task and then drives the manager loop to completion, which \
     only the CLI runs. Use `nestlone fleet restart <worker-id>`.";
...
/// `queued` receipt with a ticket. See `nestlone-tui::lane_control`.
...
pub static OPERATIONS: &[OperationDescriptor] = &[
    OperationDescriptor {
        ...
        slash_command: "lane",
        cli_invocation: "nestlone lane list",
    ...
    OperationDescriptor {
        ...
        slash_command: "lane",
        cli_invocation: "nestlone lane status <lane-id>",
    ...
    OperationDescriptor {
        ...
        hotbar_bare_dispatch: false,
        slash_command: "lane",
        cli_invocation: "nestlone lane interrupt <lane-id>",
    ...
    OperationDescriptor {
        ...
        hotbar_bare_dispatch: false,
        slash_command: "lane",
        cli_invocation: "nestlone lane restart <lane-id>",
    ...
    OperationDescriptor {
        ...
        hotbar_bare_dispatch: false,
        slash_command: "lane",
        cli_invocation: "nestlone lane resume <lane-id>",
    ...
    OperationDescriptor {
        ...
        hotbar_bare_dispatch: false,
        slash_command: "fleet",
        cli_invocation: "nestlone fleet list",
    ...
    OperationDescriptor {
        ...
        hotbar_bare_dispatch: false,
        slash_command: "fleet",
        cli_invocation: "nestlone fleet status",
    ...
    OperationDescriptor {
        ...
        hotbar_bare_dispatch: false,
        slash_command: "fleet",
        cli_invocation: "nestlone fleet interrupt <worker-id>",
    ...
    OperationDescriptor {
        ...
        hotbar_bare_dispatch: false,
        slash_command: "fleet",
        cli_invocation: "nestlone fleet restart <worker-id>",
    ...
    OperationDescriptor {
        ...
        hotbar_bare_dispatch: false,
        slash_command: "fleet",
        cli_invocation: "nestlone fleet resume <run-id>",
...
pub struct ControlReceipt {
    ...
    ///
    /// Deliberately **not** serialized: it exists so `nestlone lane
...
///
/// `nestlone lane …`, `/lane …`, and the hotbar dispatch of `/lane` all call
    ...
    if execution.reconciles() {
        ...
        warnings.push(
            "runtime reconciliation skipped on this surface; statuses are as last recorded. \
             Run `nestlone lane list` for a reconciled view."
                .to_string(),
        );
    }
    ...
    if execution.reconciles() {
        ...
        detail.push(
            "runtime reconciliation skipped on this surface; status is as last recorded. \
             Run `nestlone lane status <lane-id>` for a reconciled view."
                .to_string(),
        );
    }
...
mod tests {
    ...
    fn both_surfaces_map_to_the_same_operation_ids() {
        ...
        for descriptor in OPERATIONS {
            ...
            assert!(
                descriptor.cli_invocation.starts_with("nestlone "),
    ...
    /// answer is the off-loop executor in `nestlone-tui::lane_control` — not a
