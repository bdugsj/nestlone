...
pub fn is_upstream_auth_header(name: &str) -> bool {
    ...
    // generated by Nestlone itself. `auth_mode = "none"` is an endpoint
...
pub struct ProviderConfigToml {
    ...
    /// Nestlone-owned xAI OAuth generation selected by config. The value is a
    /// validated basename under `$CODEWHALE_HOME/credentials`, never an
    /// arbitrary path.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub oauth_credential_generation: Option<String>,
    /// Preserve provider fields introduced by newer Nestlone versions and by
...
/// A provider owner that Nestlone can identify with high confidence when an
...
pub fn provider_base_url_is_official(provider: ProviderKind, base_url: &str) -> bool {
    ...
    match provider {
        ...
        // Custom routes have no Nestlone-owned official endpoint. The
...
/// Resolve a state subdirectory, preferring the Nestlone root if
...
/// Ensure a state subdirectory exists under the primary Nestlone root,
