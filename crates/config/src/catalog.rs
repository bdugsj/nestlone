...
/// model + endpoint) plus the offering-owned Models.dev facts Nestlone wants to
...
/// bundled asset already uses Nestlone ids). Live refresh normalizes aliases
...
/// timestamp. Provider keys are normalized onto Nestlone [`crate::ProviderKind`]
...
fn offerings_from_models_dev(
    ...
    for (provider_key, provider) in &catalog.providers {
        ...
        let provider_id = if normalize_provider_ids {
            // Normalize Models.dev provider ids onto Nestlone kinds when known
