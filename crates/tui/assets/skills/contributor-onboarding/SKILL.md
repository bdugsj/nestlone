...
- Do not privilege any provider. Nestlone is provider-neutral; a dogfood run
...
```
./target/release/nestlone exec --help
```

This is a provider-free smoke check: it exercises the built binary without
sending a request anywhere. Anything beyond it — an actual `nestlone exec`
