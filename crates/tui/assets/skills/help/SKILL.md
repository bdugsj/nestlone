---
name: help
description: Route a "how do I use Nestlone" question to the installed help, config, and doctor surfaces instead of reciting a manual from memory. Explicit-only.
...
## When to use
Load it only when the user explicitly asks how to use Nestlone itself —
...
- Do not answer from memory of another harness; Nestlone's surfaces differ.
...
2. **Skills** — `/skills` opens the manager, `/skills inspect` prints the
   ...
   activates one. See `docs/SKILLS.md` in a Nestlone checkout.
...
5. **Environment problems** — `nestlone-tui doctor` reports the resolved
   config path, provider credential presence (never values), and workspace
   state. Prefer its output over inference.

## Working in a Nestlone checkout
When the workspace *is* a Nestlone checkout, `docs/` is present on disk and
