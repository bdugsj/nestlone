...
/// 2. Writes a crash dump to `~/.nestlone/crashes/<timestamp>-<name>.log`.
///
...
/// best-effort crash dump to `~/.nestlone/crashes/`, so diagnostics land in
...
/// Write a panic dump file to `~/.nestlone/crashes/`.
///
...
fn write_panic_dump(
    ...
    let crash_dir = home.join(".nestlone").join("crashes");
    if !crash_dir.exists() {
...
/// panics, a crash dump is written to `~/.nestlone/crashes/` and the panic
