...
//!
//! Nestlone already assembles prompts most-static → most-volatile for prefix
...
mod tests {
    ...
    fn snapshot_renders_constitution_then_world_state_blocks() {
        ...
        let snapshot = WorldStateSnapshot {
            constitution: "You are Nestlone.".to_string(),
            world_state: state,
        };
        let blocks = snapshot.to_system_blocks();
        assert_eq!(blocks.len(), 3);
        assert_eq!(blocks[0].text, "You are Nestlone.");
        ...
        assert!(text.starts_with("You are Nestlone."));
