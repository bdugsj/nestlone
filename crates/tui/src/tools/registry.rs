...
impl ToolRegistryBuilder {
    ...
    /// Include security analysis tools from nestlone:
    /// `hex_dump`, `extract_strings`, `base64_encode`, `base64_decode`,
    /// `hex_decode`, `tea_decrypt`.
    #[must_use]
    pub fn with_security_analysis_tools(self) -> Self {
        use super::binary_analysis::{
            Base64DecodeTool, Base64EncodeTool, ExtractStringsTool, HexDecodeTool, HexDumpTool,
            TeaDecryptTool,
        };
        self.with_tool(Arc::new(HexDumpTool))
            .with_tool(Arc::new(ExtractStringsTool))
            .with_tool(Arc::new(Base64DecodeTool))
            .with_tool(Arc::new(Base64EncodeTool))
            .with_tool(Arc::new(HexDecodeTool))
            .with_tool(Arc::new(TeaDecryptTool))
    }
