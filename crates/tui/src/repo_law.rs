...
//!
//! `.nestlone/constitution.json` invariants were previously advisory prose
...
pub(crate) fn repo_law_plan_decision(
    ...
    let reason = format!(
        "Repo law holds this write: \"{}\" protects {protects} (matched {target}, .nestlone/constitution.json)",
        rule.text
    );
...
mod tests {
    ...
    fn write_law(workspace: &Path, body: &str) {
        let dir = workspace.join(".nestlone");
    ...
    fn block_action_denies_protected_write() {
        ...
        assert!(reason.contains(".nestlone/constitution.json"), "{reason}");
    }
