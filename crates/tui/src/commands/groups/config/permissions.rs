...
use nestlone_config::{PermissionsFileState, PermissionsSnapshot, ToolAskRule};
...
fn remove_permission(app: &App, parts: &[&str]) -> CommandResult {
    ...
    let removed =
        match nestlone_config::remove_permission_rule(app.config_path.clone(), index, parts[3]) {
            Ok(rule) => rule,
            Err(error) => return operation_error(app, &error),
        };
...
fn load_snapshot(app: &App) -> anyhow::Result<PermissionsSnapshot> {
    nestlone_config::load_permissions_snapshot(app.config_path.clone())
}
...
fn format_snapshot(app: &App, snapshot: &PermissionsSnapshot) -> String {
    ...
    let path = nestlone_config::quote_os_path(snapshot.path());
...
fn rule_applies_in_workspace(rule: &ToolAskRule, workspace: &std::path::Path) -> bool {
    ...
    let Some(rule_workspace) = nestlone_execpolicy::normalize_workspace_scope(rule_workspace)
    else {
        return false;
    };
    let Some(workspace) = nestlone_execpolicy::normalize_workspace_scope(&workspace) else {
        return false;
    };
...
"#,
        ...
        let displayed_permissions_path =
            nestlone_config::resolve_permissions_path(Some(config_path.clone()))
    ...
    fn remove_requires_preview_token_then_emits_live_reload_action() {
        ...
        let parsed: nestlone_config::PermissionsToml =
