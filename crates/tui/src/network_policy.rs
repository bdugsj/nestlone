...
//!    `~/.nestlone/audit.log` in the format described below.
//!
...
//! `~/.nestlone/audit.log`). Best-effort: write failures are logged but do
//! not block the call.
...
impl NetworkAuditor {
    ...
    /// Auditor pointing at `~/.nestlone/audit.log`. Returns `None` if the
    ...
    pub fn record(&self, host: &str, tool: &str, decision_label: &str) {
        ...
        if let Err(err) = self.try_record(host, tool, decision_label) {
            ...
            // `~/.nestlone/logs/tui-YYYY-MM-DD.log` rather than the
...
impl NetworkPolicyDecider {
    ...
    /// `~/.nestlone/audit.log`, if `policy.audit` is true.
    #[must_use]
