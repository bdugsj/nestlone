//! Table-driven registries for `nestlone remote-setup`.
//!
...
pub enum SecretStore {
    /// Secrets live in `/etc/nestlone/*.env` files on the host.
    EnvFile,
...
impl SecretStore {
    ...
    pub fn label(self) -> &'static str {
        match self {
            SecretStore::EnvFile => "EnvFile (/etc/nestlone/*.env)",
...
pub const TELEGRAM: BridgeSpec = BridgeSpec {
    ...
    service_unit: "nestlone-telegram-bridge.service",
    ...
    install_dir: "/opt/nestlone/telegram-bridge",
};
...
pub const FEISHU: BridgeSpec = BridgeSpec {
    ...
    service_unit: "nestlone-feishu-bridge.service",
    ...
    install_dir: "/opt/nestlone/bridge",
};
...
/// `/etc/nestlone/*.env`, native+systemd install driven by a cloud-init
...
fn lighthouse_plan(inputs: &DeployInputs) -> Vec<ProvisionStep> {
    ...
    let restart_bridge = format!("nestlone-{}-bridge", inputs.bridge_slug);
    vec![
        ProvisionStep::new(
...
fn azure_plan(inputs: &DeployInputs) -> Vec<ProvisionStep> {
    ...
    let provider_secret = format!("nestlone-{}-key", inputs.provider_slug);
    vec![
        ProvisionStep::new(
...
fn digitalocean_plan(inputs: &DeployInputs) -> Vec<ProvisionStep> {
    ...
    vec![
        ...
        ProvisionStep::new(
            "On the Droplet: write /etc/nestlone/*.env, install both systemd units, enable --now",
            "bash",
            &["scripts/tencent-lighthouse/install-services.sh"],
        ),
    ]
