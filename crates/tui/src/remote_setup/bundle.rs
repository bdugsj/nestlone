//! Bundle rendering for `nestlone remote-setup`.
//!
...
pub fn render_bundle(inputs: &BundleInputs) -> Vec<RenderedFile> {
    vec![
        ...
        RenderedFile {
            relative_path: "nestlone-runtime.service".to_string(),
...
fn render_runtime_env(i: &BundleInputs) -> String {
    ...
    out.push_str("# Nestlone runtime config — generated by `nestlone remote-setup`.\n");
...
fn render_bridge_env(i: &BundleInputs) -> String {
    let mut out = String::new();
    out.push_str(&format!(
        "# Nestlone {} bridge config — generated by `nestlone remote-setup`.\n",
        i.bridge.display
    ));
    ...
    out.push_str("\n# --- bridge tuning ---\n");
    out.push_str(&format!(
        "{}_THREAD_MAP_PATH=/var/lib/nestlone-{}-bridge/thread-map.json\n",
...
fn render_runtime_unit(i: &BundleInputs) -> String {
    format!(
        "[Unit]\n\
Description=Nestlone Runtime API\n\
...
# Legacy /etc/deepseek is loaded first for old installs; /etc/nestlone wins.\n\
EnvironmentFile=-/etc/deepseek/runtime.env\n\
EnvironmentFile=-/etc/nestlone/runtime.env\n\
ExecStart=/bin/sh -lc 'exec /home/nestlone/.cargo/bin/nestlone serve --http --host 127.0.0.1 --port \"${{CODEWHALE_RUNTIME_PORT:-${{DEEPSEEK_RUNTIME_PORT:-{port}}}}}\" --workers \"${{CODEWHALE_RUNTIME_WORKERS:-${{DEEPSEEK_RUNTIME_WORKERS:-{workers}}}}}\" --auth-token \"${{CODEWHALE_RUNTIME_TOKEN:-${{DEEPSEEK_RUNTIME_TOKEN}}}}\"'\n\
Restart=on-failure\n\
RestartSec=5\n\
NoNewPrivileges=true\n\
PrivateTmp=true\n\
ProtectSystem=full\n\
ReadWritePaths=/home/nestlone/.nestlone /home/nestlone/.deepseek {workspace}\n\n\
[Install]\n\
...
fn render_bridge_unit(i: &BundleInputs) -> String {
    format!(
        "[Unit]\n\
Description=Nestlone {display} Phone Bridge\n\
Wants=network-online.target nestlone-runtime.service\n\
After=network-online.target nestlone-runtime.service\n\n\
[Service]\n\
Type=simple\n\
User=nestlone\n\
Group=nestlone\n\
...
EnvironmentFile=-/etc/nestlone/{slug}-bridge.env\n\
...
PrivateTmp=true\n\
ProtectSystem=full\n\
ReadWritePaths=/var/lib/nestlone-{slug}-bridge\n\n\
[Install]\n\
WantedBy=multi-user.target\n",
...
fn render_runbook(i: &BundleInputs) -> String {
    ...
    out.push_str(&format!(
        "# Nestlone remote-setup runbook — {} + {}\n\n",
        i.cloud.display, i.bridge.display
    ));
    out.push_str("Generated by `nestlone remote-setup` (generate-only). Nothing was run on\n");
    ...
    out.push_str("| `nestlone-runtime.service` | systemd unit for the runtime API. |\n");
    out.push_str(&format!(
    ...
    out.push_str("```sh\nsudo install -d -m 750 /etc/nestlone\n");
    out.push_str(&format!(
        "sudo install -m 600 runtime.env /etc/nestlone/runtime.env\n\
sudo install -m 600 {slug}.env /etc/nestlone/{slug}-bridge.env\n\
sudo install -m 644 nestlone-runtime.service /etc/systemd/system/nestlone-runtime.service\n\
sudo install -m 644 {unit} /etc/systemd/system/{unit}\n\
sudo systemctl daemon-reload\n\
sudo systemctl enable --now nestlone-runtime {unit}\n```\n\n",
        slug = i.bridge.slug,
    ...
    out.push_str("```sh\nsudo systemctl status nestlone-runtime --no-pager\n");
    out.push_str(&format!(
...
mod tests {
    ...
    fn bundle_renders_expected_file_set() {
        ...
        assert!(names.contains(&"nestlone-runtime.service"));
        assert!(names.contains(&"nestlone-feishu-bridge.service"));
    ...
    fn systemd_units_reference_nestlone_paths() {
        ...
        let unit = &files
            .iter()
            .find(|f| f.relative_path == "nestlone-runtime.service")
            .unwrap()
            .contents;
        assert!(unit.contains("/etc/nestlone/runtime.env"));
        ...
        let bridge_unit = &files
            .iter()
            .find(|f| f.relative_path == "nestlone-feishu-bridge.service")
            .unwrap()
            .contents;
        assert!(bridge_unit.contains("/etc/nestlone/feishu-bridge.env"));
    }
}
