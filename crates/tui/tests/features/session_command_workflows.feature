...
# cargo test -p nestlone-tui --bin nestlone-tui --features long-running-tests commands::groups::session::acceptance -- --test-threads=1
Feature: Session command workflows

  Scenario: Save and export preserve data while load defers restoration
    Given a Nestlone session workspace with one user message
    ...
    And Nestlone should defer the session-loaded receipt to the event loop

  Scenario: Fork keeps the original session resumable
    Given a Nestlone persisted session workspace with one user message
  ...
  Scenario: New session cannot be forked before messages exist
    ...
    Then Nestlone should reject the fork because there are no messages
  ...
  Scenario: Sessions prune removes only stale sessions
    Given a Nestlone session workspace with stale and fresh saved sessions
    When the user prunes sessions older than 7 days
    Then Nestlone should report that one session was pruned
