...
test("commands map common actions while menu/start stay opt in", () => {
  assert.deepEqual(parseCommand("/allow@NestloneBot ap_1 remember", { stripBotMention: true }), {
    name: "allow",
    args: "ap_1 remember"
  });
  assert.deepEqual(parseCommand("/allow@NestloneBot ap_1 remember"), {
    name: "allow@nestlonebot",
    args: "ap_1 remember"
  });
...
test("ThreadStore supports chat state, message dedupe, and action tokens", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "nestlone-bridge-core-"));
  try {
